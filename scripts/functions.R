...
initPop <- function(N){
  ...
  pop <- cbind(patch, n, A) # Binds all the vectors into a matrix
...
# Randomly wipes out particular patches according to an extinction probability parameter
wipe <- function(pop){
  patch <- unique(pop[,"patch"]) # Creates a vector of all occupied patches
  trackWipe <- sample(1:1/ex.prob, replace = TRUE, size = length(patch)) # Generates a random value for each patch
  wipeMat <- cbind(patch,trackWipe) # A matrix of the patches to be wiped. Patches with 1 will be wiped
  pop <- as.matrix(merge(pop, wipeMat)) # Assigns previously generated random values to rows according to patch. as.matrix() magics away the data frame that merge() creates.
  pop <- subset(pop, pop[,"trackWipe"]!=1) # Deletes rows/individuals that are in patches marked for extinction
  pop <- pop[, c("patch", "n", "A")] # Removes now redundant column that tracks extinction
  pop # The final population sans individuals from wiped patches
}
...
pGrowth <- function(pop){
  ...
  pop <- wipe(pop) # Causes all animals in a random selection of patches to die
